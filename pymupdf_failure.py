...
greenburgh = pymupdf.open('Town of Greenburgh.pdf')
...
# Data quality of greenburgh is very poor. Text seem to have arbitrary positions, even if I extract it as words and coordinates. Text extracted as text has the same effect.

# In[71]:


# Page extraction example
import tkinter as tk

def show_words(coords, words):
    root = tk.Tk() 
    ...
    # Creating label
    labels = []
    for coord,word in list(zip(coords,words)):
        hello = tk.Label(root, text=word,bg='#000000') 
        hello.place(x=coord[0], y=coord[1])
     
    root.mainloop()  

def inspect(data):# Inspect the data
    page = data.load_page(0)
    page_text = page.get_text('words')

    alist = []; blist = []
    for word in page_text:
        alist.append(word[0:2])
        blist.append(word[2:4])
        
    xs=[];ys=[]
    for clist in (alist,blist):
        for coord in clist:
            xs.append(coord[0])
            ys.append(coord[1])

    words = []
    for i in page_text: words.append(i[4])

    print(list(zip(alist,words))[1])
    show_words(alist,words)

    #page_text = page.get_text('dict')
    #col1 = 0
    #col2 = 1
    #for j in range(len(page_text['blocks'])):
    #    print(f"\nblock {j}")
    #    for i in range(len(page_text['blocks'][j]['lines'])):
    #        print([page_text['blocks'][j]['lines'][i]['spans'][0]['text']])

inspect(greenburgh)


# Trying to extract text as blocks, and it looks pretty much the same as dict, words and text.
...
# And html returns the same thing
def inspect(data):# Inspect the data
    p = 0
    page = data.load_page(p)
    page_text = page.get_text('html')

    print(len(page_text))

    with open(f"page{p}.html",'w') as f:
        f.write(page_text)
...
def inspect(data):# Inspect the data
    ...
    myc = []; mychars = []
    xs = []; ys = []
    for block in range(len(page_text['blocks'])):
        for line in range(len(page_text['blocks'][block]['lines'])):
            for char in page_text['blocks'][block]['lines'][line]['spans'][0]['chars']:
                #print(char['c'],end="")
                #mychars.append((char['origin'],char['c']))
                myc.append(char['origin'])
                mychars.append(char['c'])
                xs.append(char['origin'][0])
                ys.append(char['origin'][1])
            #print(' ',end="")
        #print()
    print(max(xs),max(ys))

    show_words(myc,mychars)
inspect(greenburgh)
